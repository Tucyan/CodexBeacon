...
if ($existing) { Write-Host 'Codex Beacon is already running. Open Settings from its tray icon.'; return }
$process = Start-Process -FilePath $executable -WorkingDirectory $workspace -WindowStyle Hidden -PassThru
Write-Host "Started Codex Beacon. PID: $($process.Id). Use its tray icon for Settings/Show/Hide/Quit."
